...
export default function ProyectosPage() {
  const renderStructuredProposal = (text: string | null | undefined) => {
    // This is now handled by ProposalPrintLayout, but kept here if used elsewhere in the file.
    // Wait, let's actually remove it and use ProposalPrintLayout where needed, 
    // or keep a simple stub if we can't remove it yet to avoid breaking stuff.
    // I will remove it because we only use it for printing which will be replaced.
  };
                        ...
                        <>
                          {project.status === 'in_progress' || project.status === 'completed' ? (
                            ...
                            <div style={{ display: 'flex', gap: '0.5rem' }}>
                              ...
                              {!isObserver && !((isSales || isClient) && project.status !== 'proposal') && (
                                <button
                                  className="btn-secondary"
          ...
          <div className="card modal-content print-content animate-fade" style={{ maxWidth: '800px', width: '90%', maxHeight: '90vh', display: 'flex', flexDirection: 'column', padding: 0 }}>
            ...
            <div className="hide-on-print" style={{ padding: '1.25rem 2rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    ...
                    {isCreatorRole && !((isSales || isClient) && selectedProject.status !== 'proposal') && (
                      <button className="btn-secondary" onClick={() => {
                        if ((isSales || isClient) && selectedProject.status === 'proposal') {
                          setShowProposalModal(true);
                        } else {
